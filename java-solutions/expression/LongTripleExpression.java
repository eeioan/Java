...
/**
 * Three-argument arithmetic expression over longs.
 *
...
public interface LongTripleExpression extends ToMiniString {
    ...
    Type<Long> TYPE = new Type<>(a -> (long) a, r -> r.getRandom().nextLong(), long.class);
    ExpressionKind<LongTripleExpression, Long> KIND = new ExpressionKind<>(
            TYPE,
            LongTripleExpression.class,
            List.of(
                    Pair.of("x", new Variable("x")),
                    Pair.of("y", new Variable("y")),
                    Pair.of("z", new Variable("z"))
            ),
            (expr, variables, values) -> expr.evaluateL(values.get(0), values.get(1), values.get(2))
    );
    ...
    static ExpressionTester<?, ?> tester(final TestCounter counter) {
        ...
        return new ExpressionTester<>(
                ...
                (op, a, b) -> (x, y, z) -> op.apply(a.evaluateL(x, y, z), b.evaluateL(x, y, z)),
                Long::sum, (a, b) -> a - b, (a, b) -> a * b, (a, b) -> a / b
        )
                .basic("10", "10", (x, y, z) -> 10L, c(10))
    ...
    private static Const c(final long c) {
        return TYPE.constant(c);
    }

    static void main(final String... args) {
        TripleExpression.SELECTOR
                .variant("LongTriple", ExpressionTest.v(LongTripleExpression::tester))
                .main(args);
    }
}
